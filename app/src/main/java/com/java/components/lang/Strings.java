...
import com.java.components.util.*;
...
public class Strings {
    ...
    public String appends(String... strs) {
        return concats(strs);
    }
    ...
    public String append(String str, int index) {
        return text.substring(0, index) + str + text.substring(index, text.length());
    }
    ...
    public boolean startsWith(char c) {
        return startsWith(c, 0);
    }

    public boolean startsWith(char c, int fromIndex) {
        return c == this.text.charAt(fromIndex == 0 ? 0 : fromIndex - 1);
    }
    ...
    public boolean endsWith(char c, int fromIndex) {
        return startsWith(c, (this.text.length() + 1) - fromIndex);
    }

    public boolean startsWithIgnoreCase(String start) {
        return text.toLowerCase().startsWith(start.toLowerCase());
    }

    public boolean endsWithIgnoreCase(String end) {
        return text.toLowerCase().endsWith(end.toLowerCase());
    }

    public boolean startsEndsWith(String start, String end) {
        return text.startsWith(start) && text.endsWith(end);
    }

    public boolean startsEndsWith(String start, int toffsetStart, String end) {
        return text.startsWith(start, toffsetStart) && text.endsWith(end);
    }

    public boolean startsEndsWith(String start, String end, int toffsetEnd) {
        return text.startsWith(start) && endsWith(end, toffsetEnd);
    }

    public boolean startsEndsWith(String start, int toffsetStart, String end, int toffsetEnd) {
        return text.startsWith(start, toffsetStart) && endsWith(end, toffsetEnd);
    }

    public boolean startsEndsWith(int toffsetAll, String start, String end) {
        return text.startsWith(start, toffsetAll) && endsWith(end, toffsetAll);
    }

    @Deprecated public boolean startsEndsWith(int toffsetAll, String start, int toffsetStart, String end, int toffsetEnd) {
        return text.startsWith(start, toffsetAll + toffsetStart) && endsWith(end, toffsetAll + toffsetEnd);
    }

    @Deprecated public boolean startsEndsWith(int toffsetAll, String start, String end, int toffsetEnd) {
        return text.startsWith(start, toffsetAll) && endsWith(end, toffsetAll + toffsetEnd);
    }

    @Deprecated public boolean startsEndsWith(int toffsetAll, String start, int toffsetStart, String end) {
        return text.startsWith(start, toffsetAll + toffsetStart) && endsWith(end, toffsetAll);
    }
    ...
    public String replaceBegin(String targetBegin, String replacement) {
        if(text.startsWith(targetBegin))
            return replacePrefix(targetBegin.length(), replacement);
        return text;
    }

    public String replaceEnd(String targetEnd, String replacement) {
        if(text.endsWith(targetEnd))
            return replaceSuffix(shortLength(targetEnd.length()), replacement);
        return text;
    }

    public String replacePrefix(int start, String replacement) {
        return replace(0, start, replacement);
    }

    public String replaceSuffix(int end, String replacement) {
        return replace(shortLength(end), text.length(), replacement);
    }

    public String replace(String target, Function<String, String> replacement) {
        return text.replace(target, replacement.apply(target));
    }
    ...
    public boolean endsWith(String suffix, int toffset) {
        return text.startsWith(suffix, text.length() - suffix.length() - toffset);
    }
    ...
    public String[] cut(int cutIndex) {
        return split(cutIndex, 2);
    }

    public String slide(int startIndex) {
        if(startIndex < 0) return slide(startIndex, 0);
    ...
    public String prefix(int prefix) {
        return text.substring(0, prefix);
    }

    public String suffix(int suffix) {
        return text.substring(text.length() - suffix, text.length());
    }

    public String substring(String string) {
        return substring(string, false);
    }
    ...
    public String substr(int index, int length) {
        return substring(index, index + length, false);
    }

    public String substr(int index) {
        return substring(index, text.length(), false);
    }
    ...
    public String formatted(Object... args) {
        return new Formatter().format(text, args);
    }
    
    public static String format(String format, Object... args) {
        return new Strings(format).formatted(args);
    }

    public static String join(CharSequence prefix, CharSequence suffix, CharSequence delimiter, CharSequence... args) {
        return join(prefix, suffix, delimiter, 0, args);
    }

    public static String join(CharSequence delimiter, int limit, CharSequence... args) {
        return join("", "", delimiter, limit + 1, args);
    }

    public static String join(CharSequence prefix, CharSequence suffix, CharSequence delimiter, int limit, CharSequence... args) {
        return join(prefix, suffix, delimiter, -1, limit + 1, args);
    }
    ...
    public static String join(CharSequence prefix, CharSequence suffix, CharSequence delimiter, int limitLengthCharSequence, int limit, CharSequence... args) {
        ...
        for(int i = 0; i < limit; i++) {
            if(limitLengthCharSequence == args[i].length() || limitLengthCharSequence == -1) parts.append(((String) prefix) + ((String) args[i]) + ((String) suffix) + ((String) delimiter));
        }
    ...
    public String difference(String other) {
        ...
        for(int i = 0; i < Math.min(text.length(), other.length()); i++) {
            if(text.charAt(i) != other.charAt(i)) {
                diff.append(text.charAt(i) + " != " + other.charAt(i) + ", index: " + i + "\n");
            }
        }
    ...
    private void setValue(Matcher matcher, StringBuffer stringBuilder, Object valueField) {
        if (valueField instanceof String v) {
            matcher.appendReplacement(stringBuilder,  v);
        } else if (valueField instanceof Character v) {
            matcher.appendReplacement(stringBuilder,  String.valueOf(v));
        } else if (valueField instanceof Boolean v) {
            matcher.appendReplacement(stringBuilder,  String.valueOf(v));
        } else if (valueField instanceof Integer v) {
            matcher.appendReplacement(stringBuilder,  String.valueOf(v));
        } else if (valueField instanceof Short v) {
            matcher.appendReplacement(stringBuilder,  String.valueOf(v));
        } else if (valueField instanceof Byte v) {
            matcher.appendReplacement(stringBuilder,  String.valueOf(v));
        } else if (valueField instanceof Long v) {
            matcher.appendReplacement(stringBuilder,  String.valueOf(v));
        } else if (valueField instanceof Double v) {
            matcher.appendReplacement(stringBuilder,  String.valueOf(v));
        } else if (valueField instanceof Float v) {
            matcher.appendReplacement(stringBuilder,  String.valueOf(v));
        }
    }

    public int shortLength(int cutLength) {
