...
import com.java.components.lang.Strings;
...
public class StringTokenizer {
	...
	public StringTokenizer(String str, String delimiter, boolean returnDelim, int limit) {
		/* OLD:
			...
			if(delimiter.length() > 1) {
				...
				for (int i = 0; i < delimiterParts.length; i++) sb.append(delimiterParts[i]).append("|");
				if(sb.length() > 2) sb.delete(sb.length() - 1, sb.length());
			} else {
				sb.append(delimiter);
			}
			if(limit >= 1) {
				this.tokens = str.split(sb.toString().replace(".", "\\."), limit += 1);
			} else {
				this.tokens = str.split(sb.toString().replace(".", "\\."));
			}
	...
	public String nextTokenAndSkip(String skip) {
		...
		Strings s = new Strings(skipText);
		return s.skip(skip);
	}
	...
	public String previouTokenAndSkip(String skips) {
		...
		Strings s = new Strings(textSkip);
		return s.skip(skips);
	}
	...
	public String getNextTokensAndSkip(String textSkip) {
		StringBuilder sb = new StringBuilder();
		while (hasNextMoreTokens()) sb.append(nextToken());
		return new Strings(sb.toString()).skip(textSkip);
	}
	...
	public String getPreviouTokensAndSkip(String skipText) {
		...
		return new Strings(sb.toString()).skip(skipText);
	}
